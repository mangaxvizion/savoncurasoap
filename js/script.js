...
/* =====================================================
   ...
   - la vidéo au centre se lance toute seule, dans la page (jamais en plein écran)
   - le son est tenté automatiquement ; si le navigateur le refuse, la vidéo
     démarre en muet et le son s'active tout seul au premier toucher (aucun bouton)
...
(function reel(){
  ...
  let visible = false;   // le carrousel est à l'écran (mis à jour à l'affichage)
  ...
  slides.forEach((s, i) => {
    ...
    // lecture dans la page (TikTok, Instagram, Facebook, navigateurs Android) : pas de plein écran
    ...
    // toucher : vidéo voisine → on la lance ; vidéo au centre → play / pause
    s.addEventListener('click', () => {
      if (i !== nearest()){
        const cur = activeVideo(); if (cur) cur.pause();
        userPlay(v);
        goTo(i);
        return;
      }
      if (v.paused){ userPlay(v); }
      else if (!blocked){ manualPause = true; v.pause(); }   // (si le son est encore bloqué, le toucher active le son)
    });
  });
  ...
  // lecture demandée par un toucher : le son est autorisé
  function userPlay(v){
  ...
  async function playActive(){
    ...
    // précharge la vidéo courante et la suivante (seulement quand on les regarde)
    [current, (current + 1) % slides.length].forEach(k => { $('video', slides[k]).preload = 'auto'; });
    v.muted = blocked;                       // on essaie d'abord AVEC le son
    try { await v.play(); }
    catch (err){
  ...
  // premier toucher : on active le son en silence (sans bouton)
  function tryUnmute(){
    if (!blocked || manualPause) return;
    const v = activeVideo();
    if (!v || !visible || voiceOn) return;
...
(function order(){
  ...
  // ---- panneau de secours (si WhatsApp ne s'ouvre pas, ex. dans TikTok) ----
  const panel = $('#waPanel');
  const prettyNumber = '+' + WHATSAPP_NUMBER.slice(0, 3) + ' ' + WHATSAPP_NUMBER.slice(3).replace(/(\d{2})(?=\d)/g, '$1 ');
  let lastMsg = '';

  function showPanel(msg, webUrl, appUrl){
    lastMsg = msg;
    $('#waApp').href = appUrl;
    $('#waWeb').href = webUrl;
    $('#waNum').textContent = prettyNumber;
    $('#waCopy').textContent = 'Copier ma commande';
    $('#waTip').hidden = !/TikTok|musical_ly|Bytedance|trill/i.test(navigator.userAgent);
    panel.hidden = false;
  }
...
🧭 Point de repère : ${$('#fLandmark').value.trim()}`;
    ...
    const webUrl = 'https://wa.me/' + WHATSAPP_NUMBER + '?text=' + enc;
    const appUrl = 'whatsapp://send?phone=' + WHATSAPP_NUMBER + '&text=' + enc;

    btn.disabled = true; label.textContent = 'Ouverture de WhatsApp…';

    // Ouverture immédiate, dans le geste de l'utilisateur, en nouvelle fenêtre :
    // c'est ce qui évite la page intermédiaire bloquée dans TikTok.
    try { window.open(webUrl, '_blank'); } catch (err){}

    // Filet de sécurité : si la page est toujours affichée, on propose l'ouverture manuelle.
    setTimeout(() => {
      btn.disabled = false; label.textContent = original;
      if (document.visibilityState === 'visible') showPanel(msg, webUrl, appUrl);
    }, 1500);
  });
